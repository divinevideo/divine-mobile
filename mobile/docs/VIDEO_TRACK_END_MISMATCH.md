...
| `lib/screens/video_editor/video_clip_chroma_key_screen.dart`, `lib/screens/video_editor/video_clip_transform_screen.dart`, `lib/widgets/video_editor/main_editor/video_editor_canvas.dart` (single-clip trim-drag preview) | default `false` | Yes. All three pass an explicit `end` derived from `clip.duration`, and a second native clamp could shorten playback again and make the timeline endpoint unreachable. Note the scope of the first half of that argument: only a **recorded** clip stores a corrected common track end in `duration` — `commonTrackEnd` has exactly three callers, in `video_recorder_bloc.dart` and `clip_recovery_service.dart`. An **imported** clip takes `metadata.duration` (`lib/services/video_clip_import_service.dart`, `_durationFor`) and a **re-rendered** clip takes `metaData.duration` (`lib/services/video_editor/video_editor_render_service.dart`), both raw container durations. For those two, `end` can sit past the audio track end. Leaving the flag off is still right here — the clamp is not the fix for that — but the mismatch is real and belongs to the export path in the first half of this document, not to this row. |
