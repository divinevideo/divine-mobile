...
| `lib/screens/comments/widgets/video_comment_player.dart` | `true` | Yes. It loops one finished clip without a duration axis, so hiding a short track-end seam matches feed playback. It plays on tap, which is why it once needed the `full` buffer profile to have Android read the remote clip's track lengths before the load (#8897); Android now clips at the track end before the first frame on every profile. `video_comment_player_test.dart` pins the flag. |
...
Platform support is per-backend and documented on the field. Web and Linux ignore it. Apple reads both tracks before building the composition, local or remote, and starts a looping clip at its first frame when an empty edit of at most 100 ms delays the picture — `AVPlayerLooper` otherwise held the last frame ~200 ms at every restart. Android takes the track lengths from its own extractor while it parses the container during prepare (`TrackEndCapturingExtractorsFactory`) and clips the source there (`CommonTrackEndMediaSource`), before the first frame, on every source and every `VideoBufferProfile`, with no metadata read in front of the load. The same parse reports where the first frame is shown, and a clip whose picture an empty edit of at most 100 ms delays starts there instead of at zero: every derivative carries a 21–23 ms empty video edit, and media3 honours it, so each restart held the previous lap's last frame that much longer. Apple needs no such trim — the edit costs nothing under `AVPlayerLooper` (divinevideo/divine-blossom#235). Until then Android read the lengths a second time with a `MediaExtractor`: a `full` player waited for that read (#8897), and a `feed` player started at once and installed a late clamp at the first loop restart by swapping its item, which re-prepared the source and froze that restart for ~300 ms on every video opened from a grid or started cold. Neither platform clamps an HLS source: an HLS asset exposes no tracks to Apple, and a playlist has no container for Android to read them from.
