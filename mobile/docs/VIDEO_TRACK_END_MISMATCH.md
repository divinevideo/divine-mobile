...
| `lib/screens/comments/widgets/video_comment_player.dart` | `true` | Yes. It loops one finished clip without a duration axis, so hiding a short track-end seam matches feed playback. Its controller keeps the default `VideoBufferProfile.full`, which is what makes Android read the remote clip's track lengths *before* the load: it plays on tap, so the feed's background warm would have nothing paused to tighten and the play the user just started would keep its seam (#8897). `video_comment_player_test.dart` pins the profile next to the flag. |
...
Platform support is per-backend and documented on the field. Web and Linux ignore it. Apple reads both tracks before building the composition, local or remote. Android reads a local file before the load; for a remote source, whether the read sits in front of the load follows the controller's `VideoBufferProfile` (#8897): a `full` player — the comment player, and every preview — reads first, bounded by a short wait after which it plays unclamped, while a `feed` player starts immediately and warms the lengths in the background. That warm tightens the playlist only while the player still sits paused at frame zero, which is how the feed preloads its tiles; a feed tile that is already playing when the read lands — the current tile on a cold start, or after a failover — keeps the seam for that play, and the cached lengths clamp the next load of the source. Neither platform probes an HLS source: an HLS asset exposes no tracks to Apple, and Android's `MediaExtractor` cannot open a playlist.
