...
Platform support is per-backend and documented on the field. Web and Linux ignore it. Apple reads both tracks before building the player item, local or remote. Android takes the track lengths from its own extractor while it parses the container during prepare (`TrackEndCapturingExtractorsFactory`) and clips the source there (`CommonTrackEndMediaSource`), before the first frame, on every source and every `VideoBufferProfile`, with no metadata read in front of the load. Until then Android read the lengths a second time with a `MediaExtractor`: a `full` player waited for that read (#8897), and a `feed` player started at once and installed a late clamp at the first loop restart by swapping its item, which re-prepared the source and froze that restart for ~300 ms on every video opened from a grid or started cold. Neither platform clamps an HLS source: an HLS asset exposes no tracks to Apple, and a playlist has no container for Android to read them from.
