...
| Traffic | Why not |
|---|---|
| **Relay WebSockets** — every Nostr subscription and publish over `wss://` | Not HTTP requests; `HttpMetric` cannot express them, so where they are measured at all it is as custom traces rather than network requests. Feed subscribes are covered by the `feed_load_*` traces ([Feed-load performance traces](FEED_LOAD_TRACES.md)) and publishes by `video_publish`. Relay latency on its own — separated from the cache lookup and subscription setup those spans also cover — is still not measured. |
