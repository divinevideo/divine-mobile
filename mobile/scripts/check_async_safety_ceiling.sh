#!/usr/bin/env bash
# Async-safety analyzer ratchet (#9118): unawaited_futures and
...
NEW_HINT="Await the future, return it, or explicitly mark an intentional fire-and-forget operation with unawaited(). Do not raise this baseline. See #9118."
STALE_HINT="Async-safety findings were removed."
FOOTER="unawaited_futures and discarded_futures are frozen per rule and file.
The #9118 owner and each affected file's owner must drive these counts to zero."
...
emit_current() {
  ...
  if [[ -z "$output_file" ]]; then
    ...
    if [[ -n "$still_suppressed" ]]; then
      ...
      echo "     the tracked async-safety debt. See #9118." >&2
      return 1
    fi
...
# Owner: issue #9118 (NotThatKindOfDrLiz); affected-file owners must classify
