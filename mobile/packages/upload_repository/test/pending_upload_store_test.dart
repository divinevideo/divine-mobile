...
// ABOUTME: and both cleanup routines on a package-local Hive home.
...
/// Open the uploads box the way the app adapter does, but directly on the
/// Hive home the test configured. The store depends on the opener port only,
/// so the package owns this half of the contract.
Future<Box<PendingUpload>> _openUploadsBox({bool forceReinit = false}) async {
  ...
  return Hive.openBox<PendingUpload>(_boxName);
}

/// Storage that can never open, so `save()` must fall back to the deferred
/// queue and every reopen attempt fails deterministically.
Future<Box<PendingUpload>> _failingBox({bool forceReinit = false}) async {
...
PendingUploadStore _newStore({
  ...
  PendingUploadBoxOpener openBox = _openUploadsBox,
}) => PendingUploadStore(
  ...
  group(PendingUploadStore, () {
    ...
    tearDown(() async {
      try {
        ...
        // A failing-opener test can leave the registry mid-teardown; cleanup
    ...
    group('deferred save queue', () {
      ...
      test('a drain is a no-op while another drain is in flight', () async {
        // Park the drain's reopen on a gate so the second call runs while the
        // first is genuinely in flight; the immediate failing opener would
        // settle the first drain before the second call observes it.
        final gate = Completer<void>();
        var openCalls = 0;
        Future<Box<PendingUpload>> gatedBox({bool forceReinit = false}) async {
          if (openCalls++ > 0) {
            await gate.future;
          }
          throw Exception('storage unavailable');
        ...
        // Start a drain but don't await it — it suspends on the gated reopen.
