// ABOUTME: DAO for the text-overlay looks a user saved from the video
// ABOUTME: editor's timeline. CRUD, ordering, and per-account isolation.

import 'package:db_client/db_client.dart';
import 'package:drift/drift.dart';

part 'saved_title_styles_dao.g.dart';

@DriftAccessor(tables: [SavedTitleStyles])
class SavedTitleStylesDao extends DatabaseAccessor<AppDatabase>
    with _$SavedTitleStylesDaoMixin {
  SavedTitleStylesDao(super.attachedDatabase);
  ...
  Expression<bool> _ownedOrLegacy(String? ownerPubkey) {
    ...
    return savedTitleStyles.ownerPubkey.equals(ownerPubkey) |
        savedTitleStyles.ownerPubkey.isNull();
  }
  ...
  Future<List<SavedTitleStyleRow>> getStyles({String? ownerPubkey}) {
    final query = select(savedTitleStyles)
  ...
  Future<void> upsertStyle({
    ...
    return into(savedTitleStyles).insertOnConflictUpdate(
  ...
  Future<bool> renameStyle({required String id, required String name}) async {
    final rows = await (update(savedTitleStyles)..where((t) => t.id.equals(id)))
        .write(
          SavedTitleStylesCompanion(name: Value(name)),
        );
    return rows > 0;
  }
  ...
  /// Rewrite [SavedTitleStyles.orderIndex] so [orderedIds] read back in
  /// that order.
  ///
    ...
    return transaction(() async {
      for (var index = 0; index < orderedIds.length; index++) {
        await (update(
          ...
          SavedTitleStylesCompanion(orderIndex: Value(index)),
        );
      }
    });
  }

  /// The highest [SavedTitleStyles.orderIndex] currently in use, or `null`
  /// when the account has no styles yet. Callers append after it.
  Future<int?> highestOrderIndex({String? ownerPubkey}) async {
    final maxOrder = savedTitleStyles.orderIndex.max();
    final query = selectOnly(savedTitleStyles)
