...
void main() {
  group('Video Playback Stop Integration Tests', () {
    ...
    test('activeVideoId changes to null when navigating to grid mode', () async {
    ...
    test('activeVideoId changes when navigating between different feeds', () async {
      ...
      final isHomeVideoActive = container.read(isVideoActiveProvider('home-video-0'));
      final isExploreVideoActive = container.read(isVideoActiveProvider('explore-video-0'));
    ...
    test('swiping between videos in same feed changes active video', () async {
      ...
      expect(activeVideoIds, containsAllInOrder(['home-video-0', 'home-video-1']));

      locationController.close();
      container.dispose();
    });

    test('defensive behavior: defaults to background if lifecycle provider not ready', () async {
