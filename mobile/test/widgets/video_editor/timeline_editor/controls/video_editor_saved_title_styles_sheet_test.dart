// ABOUTME: Tests for the saved title styles sheet: the save / apply /
...
class _MockSavedTitleStylesCubit extends MockCubit<SavedTitleStylesState>
    implements SavedTitleStylesCubit {}
...
void main() {
  ...
  const sampleText = 'Episode 13';

  SavedTitleStyle saved(String id, String name) => SavedTitleStyle(
    id: id,
    name: name,
    style: popStyle,
  ...
  group(SavedTitleStylesSheetView, () {
    late _MockSavedTitleStyleRepository repository;
    late SavedTitleStylesCubit cubit;
    ...
    setUp(() {
      ...
      cubit = SavedTitleStylesCubit(repository: repository);
    });
    ...
    Future<TitleStyle? Function()> pumpView(
      WidgetTester tester, {
      bool disableAnimations = true,
      SavedTitleStylesCubit? cubitOverride,
    }) async {
      tester.view
          ...
          home: Scaffold(
            body: Builder(
              builder: (context) => ElevatedButton(
                onPressed: () async {
                  result = await Navigator.of(context).push<TitleStyle>(
                    MaterialPageRoute(
                      builder: (_) => BlocProvider<SavedTitleStylesCubit>.value(
                        value: cubitOverride ?? cubit,
                        child: const Scaffold(
                          body: SavedTitleStylesSheetView(
    ...
    testWidgets('shows the empty note when nothing is saved', (tester) async {
      ...
      expect(
        find.text(l10n.videoEditorTitleSavedStylesEmpty),
        findsOneWidget,
      );
      expect(
    ...
    testWidgets('renders the selected text in every preview', (tester) async {
      when(
      ...
      final previews = tester.widgetList<TitleStylePreview>(
        find.byType(TitleStylePreview),
      );
      expect(previews.map((preview) => preview.text), [sampleText, sampleText]);
      expect(previews.map((preview) => preview.style), [popStyle, popStyle]);
    });
    ...
    testWidgets('tapping a saved style pops it as the result', (tester) async {
      ...
      expect(find.byType(SavedTitleStylesSheetView), findsNothing);
    });
    ...
    testWidgets('saves the current style under the suggested font name', (
      ...
      expect(find.text(l10n.videoEditorTitleSavedStylesEmpty), findsNothing);
    });
    ...
    testWidgets('deletes a saved style after confirming', (tester) async {
      ...
      expect(
        find.text(l10n.videoEditorTitleSavedStyleDeleteConfirmMessage),
        findsOneWidget,
      );
      await tester.tap(
        find.descendant(
    ...
    testWidgets('offers a retry when the styles could not be read', (
      ...
      final failing = _MockSavedTitleStylesCubit();
  ...
  group(SavedTitleStylesSheetPage, () {
      ...
      await tester.pumpWidget(
        ProviderScope(
          ...
          child: MaterialApp(
            ...
            supportedLocales: AppLocalizations.supportedLocales,
            home: const Scaffold(
              body: SavedTitleStylesSheetPage(
                currentStyle: current,
                sampleText: sampleText,
              ),
            ),
