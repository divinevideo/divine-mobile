...
| WS-2 `Future.delayed` in tests | #4837 | Open | Executable `Future.delayed` calls under `mobile/test/**/*.dart` are frozen as per-file numeric ceilings in `mobile/scripts/baseline/future_delayed_test_counts.txt`; the production sibling `mobile/scripts/check_future_delayed_production_ceiling.sh` separately guards `mobile/lib` under #4339. |
| WS-3 untested services | #4838 | Open | `mobile/lib/services/*.dart` files without a covering test (same-named, or a `<service>_<aspect>_test.dart` that imports the service) are frozen in `mobile/scripts/baseline/untested_services.txt`. |
...
Never use `UPDATE_BASELINE=1` to add a new skipped test, add or restore a
`Future.delayed` call site, add a new untested service, or add a new file under
`mobile/test/unit`.
...
The test and production baselines store `relpath<TAB>count`. A decrease fails
until the baseline is regenerated, which locks in the lower ceiling so the
removed call cannot return. The detector strips comments, dartdoc, and string
literals before counting occurrences.
...
- Add a same-named `mobile/test/services/<service>_test.dart` when the service is
  still live and meaningful. An existing `<service>_<aspect>_test.dart` split
  suite that imports the service already covers it; do not add a thin
  basename-only file beside it.
...
`social_service` is resolved: OD-2 was decided as delete, and the service was
removed in #8760.
...
- OD-2: Resolved as delete. `mobile/lib/services/social_service.dart` had no
  callers and was removed in #8760, along with its `untested_services.txt` row.
...
1. WS-3: pay down unblocked actionable service entries.
